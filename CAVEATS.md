...
This policy does not give a container access to the host mount namespace. Bubblewrap starts each command with the container filesystem read-only, binds the writable sandbox tree at `/home/user`, and provides temporary `/tmp` and `/var/tmp` filesystems. It also gives the command private process, IPC, and mount namespaces. The command can therefore persist writes only in the sandbox tree, not in the container's initial filesystem. The container runs as a non-root user, and Compose drops all Linux capabilities. A process cannot mount in the container's initial mount namespace because it does not have `CAP_SYS_ADMIN`. A process can gain capabilities only in a new user namespace and can use them only in child namespaces that it owns. The profile does not add `setns`, so a process cannot use that system call to join another namespace. Commands can create nested user namespaces because the outer seccomp policy must permit the first user namespace that Bubblewrap creates.
